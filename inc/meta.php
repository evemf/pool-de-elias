...
function pool_de_elias_register_meta() {
    $competition_meta = [
        ...
        '_pde_points_table'    => 'array',   // ← array d'enters per posició
    ];
    ...
    foreach ( $competition_meta as $key => $type ) {
        $args = [
            'single'        => true,
            'type'          => $type,
            'auth_callback' => function() { return current_user_can( 'edit_competitions' ); },
        ];

        // Exposició a REST + esquema
        if ( 'array' === $type ) {
            // taula de punts → array d'enters
            $args['show_in_rest'] = [
                'schema' => [
                    'type'  => 'array',
                    'items' => [ 'type' => 'integer' ],
            ...
            // saneig coherent amb integers
            $args['sanitize_callback'] = function( $value ) {
                $value = is_array( $value ) ? $value : [];
                $out = [];
                foreach ( $value as $pos => $pts ) {
                    $out[ (int) $pos ] = (int) $pts;
                }
                return $out;
            };
        } else {
            // per a tipus escalares podem deixar-ho com booleà
            $args['show_in_rest'] = true;
        }

        register_post_meta( 'competition', $key, $args );
    }
    ...
    foreach ( $sub_meta as $key => $type ) {
        $args = [
            'single'        => true,
            'type'          => $type,
            'auth_callback' => function() { return current_user_can( 'edit_subcompetitions' ); },
        ];
        ...
        if ( 'array' === $type ) {
            ...
            $args['sanitize_callback'] = function( $value ) {
                ...
                $out = [];
                foreach ( $value as $pos => $pts ) {
                    // permet buit per heretar (no guardar)
                    if ( '' === $pts ) { continue; }
                    $out[ (int) $pos ] = (int) $pts;
                }
                return $out;
        ...
        register_post_meta( 'subcompetition', $key, $args );
    }
}
